...
def pause():
    ...
    while p:
        ...
        message_to_screen("press r for restart", red, 0, medium_font)
        pygame.display.update()
        for event in pygame.event.get():
            if event.type==pygame.QUIT:
                quit()
...
def level():
    loop=True
    x=int(display_length*.2)
    y=int(display_height*.30)
    radius=int(display_height*.10)
    while loop:
        for event in pygame.event.get():
            if event.type==pygame.QUIT :
                pygame.quit()
                quit()
            gameDisplay.fill(white)    
            pygame.draw.circle(gameDisplay,yellow,[x,y],radius)
            pygame.draw.circle(gameDisplay,yellow,[x+4*radius,y],radius)
            pygame.draw.circle(gameDisplay,yellow,[x+8*radius,y],radius)
            pygame.draw.circle(gameDisplay,yellow,[x,y+4*radius],radius)
            pygame.draw.circle(gameDisplay,yellow,[x+4*radius,y+4*radius],radius)
            pygame.draw.circle(gameDisplay,yellow,[x+8*radius,y+4*radius],radius)
...
def gameloop():
    ...
    while not gameexit:
                ...
                if event.key==pygame.K_LEFT :
                    ...
                    for x in range(random.randrange(10)):
                        if ( int(display_length*(.5))>enemy_main_tank_x>int((display_length*.05))): 
                            if directions[d]=='f' and enemy_main_tank_x<int(.3*display_length) and enemy_main_tank_x+(tank_width/2)<barrier_x:
                                enemy_main_tank_x+=5
        ...
        if main_tank_x-int(tank_width/2)<barrier_x+barrier_width:
            tank_move=0
        starting_fire_coordinates=tank(main_tank_x,main_tank_y,red,final_turret_pos)
        enemy_starting_fire_coordinates=enemy_tank(enemy_main_tank_x,enemy_main_tank_y,green,enemy_final_turret_pos)
        barrier_height=(display_height-barrier_y)
            ...
            if event.type==pygame.KEYDOWN:
                if event.key==pygame.K_LEFT :
                    ...
                    while loop:
                        ...
                        if enemy_main_tank_x+int(tank_width/2)>=barrier_x:
                            enemy_main_tank_x=barrier_x-int(tank_width/2)
